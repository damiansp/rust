...
/// ASCII character.  If `s` has the proper form, return `Some<(x, y)>`. If it
/// doesn't parse correctly, return `None`.
...
fn test_parse_complex() {
    assert_eq!(
        parse_complex("1.25,-0.0625"), Some(Complex {re: 1.25, im: -0.0625}));
...
fn pixel_to_point(
    ...
    let (width, height) = (
        lower_right.re - upper_left.re, upper_left.im - lower_right.im);
    Complex {
...
fn render(
        pixels: &mut [u8], bounds: (usize, usize), upper_left: Complex<f64>,
        lower_right: Complex<f64>) {
...
fn write_image(
    ...
    encoder.encode(
        pixels, bounds.0 as u32, bounds.1 as u32, ColorType::Gray(8))?;
    Ok(())
}